...
def compute_adjacency_list(input_points, input_network, id_attribute,
  ...
  for (feature, layer) in [(polygons, polygons_layer),
    ...
    arcpy.MakeFeatureLayer_management(in_features=feature, out_layer=layer)
  ...
  for row in rows:
    ...
    arcpy.Append_management(inputs=partial_adj_dbf, target=temp_adj_dbf,
        schema_type="TEST")

    progress.step()

  # Copy data from |temp_adj_dbf| to |adj_dbf|
  arcpy.Rename_management(in_data=temp_adj_dbf, out_data=adj_dbf)
